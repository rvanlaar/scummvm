...
static MethodProto xlibMethods[] = {
	{ "new",				XPlayAnim::m_new,				 0, 0,	300 },	// D3
	{ "XPlayAnim",			XPlayAnim::m_xplayanim,			 3, 3,	300 },	// D3
	{ nullptr, nullptr, 0, 0, 0 }
};

void XPlayAnim::open(int type) {
	if (type == kXObj) {
		XPlayAnimXObject::initMethods(xlibMethods);
		XPlayAnimXObject *xobj = new XPlayAnimXObject(kXObj);
...
void XPlayAnim::close(int type) {
	if (type == kXObj) {
		XPlayAnimXObject::cleanupMethods();
		g_lingo->_globalvars[xlibName] = Datum();
	}
}

XPlayAnimXObject::XPlayAnimXObject(ObjectType ObjectType) :Object<XPlayAnimXObject>("XPLayAnimXObject") {
	_objType = ObjectType;
}

void XPlayAnim::m_new(int nargs) {
	g_lingo->push(g_lingo->_currentMe);
}

void XPlayAnim::m_xplayanim(int nargs) {
